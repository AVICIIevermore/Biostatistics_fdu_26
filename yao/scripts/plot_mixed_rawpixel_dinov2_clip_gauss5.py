#!/usr/bin/env python3
"""Plot raw pixel, DINOv2, and CLIP GAUSS5 comparisons for mixed BloodMNIST experiments."""
...
def plot_panel(ax, clip: pd.DataFrame, dino: pd.DataFrame, raw: pd.DataFrame, title: str, ylabel: str) -> None:
    for (label, color, marker), frame in zip(SERIES, [clip, dino, raw], strict=True):
        ax.plot(
            frame["noise_sigma"],
            frame["value"],
            color=color,
            marker=marker,
            linewidth=2.2 if "CLIP" in label else 2.0,
            markersize=7,
            label=label,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--clip-type1-a", required=True, type=Path)
    parser.add_argument("--dino-type1-a", required=True, type=Path)
    parser.add_argument("--raw-type1-a", required=True, type=Path)
    parser.add_argument("--clip-type1-b", required=True, type=Path)
...
def main() -> None:
    ...
    plot_panel(
        axes[0],
        load_summary(args.clip_power, "power_mean"),
    ...
    plot_panel(
        axes[1],
        load_summary(args.clip_type1_a, "type1_mean"),
    ...
    plot_panel(
        axes[2],
        load_summary(args.clip_type1_b, "type1_mean"),
    ...
    fig.suptitle("BloodMNIST Mixed-Population: Raw Pixel vs DINOv2 vs CLIP (GAUSS5)", fontsize=16, weight="bold", y=1.08)
    fig.tight_layout()

    png = args.output_dir / "bloodmnist_mixed_rawpixel_vs_dinov2_vs_clip_gauss5.png"
    pdf = args.output_dir / "bloodmnist_mixed_rawpixel_vs_dinov2_vs_clip_gauss5.pdf"
