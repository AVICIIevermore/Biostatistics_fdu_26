#!/usr/bin/env python3
"""Run raw-pixel GAUSS5 MMMD experiments on an image-folder dataset."""
...
def load_pool_images(meta: pd.DataFrame, image_root: Path) -> np.ndarray:
    images = []
    for rel_path in meta["image_path"]:
        arr = np.asarray(Image.open(image_root / rel_path).convert("RGB"), dtype=np.uint8)
        images.append(arr)
    return np.stack(images, axis=0)


def flatten_images(images: np.ndarray, sigma: float, rng: np.random.Generator) -> np.ndarray:
...
def main() -> None:
    ...
    meta = meta[meta["label"].isin(keep_labels)].copy()
    if args.max_per_label is not None:
        meta = (
            meta.sort_values(["label", "id"])
            .groupby("label", group_keys=False)
            .head(args.max_per_label)
            .reset_index(drop=True)
        )
    labels = meta["label"].to_numpy(dtype=np.int64)
    images = load_pool_images(meta, Path(args.image_root).resolve())
    ...
    for outer_iter in range(1, args.n_rep + 1):
        for sigma in noise_levels:
            ...
            for inner in range(1, args.n_inner + 1):
                ...
                X = flatten_images(images[x_idx], sigma, rng)
                Y = flatten_images(images[y_idx], sigma, rng)
