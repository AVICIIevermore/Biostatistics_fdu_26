#!/usr/bin/env python3
"""Shared PathMNIST-28 CNN training and embedding pipeline.

The CNN is trained only as a 9-class classifier on the official train split.
The validation split selects the checkpoint. The test split is evaluated only
once after training for sanity reporting.
"""
...
DEFAULT_DATA_PATH = DEFAULT_WORKSPACE.parent / "data" / "pathmnist.npz"
...
def get_device(prefer_cuda: bool = True) -> str:
    if prefer_cuda and torch.cuda.is_available():
        # Match the MNIST CNN pipeline: cuDNN can fail to initialize in this shared GPU environment.
...
def plot_training_curves(history_path: Path, output_path: Path) -> None:
    ...
    axes[0].set_title("PathMNIST CNN Loss")
    axes[0].legend()
    ...
    axes[1].set_title("PathMNIST CNN Accuracy")
    axes[1].legend()
...
def save_confusion_matrix(matrix: np.ndarray, csv_path: Path, png_path: Path) -> None:
    ...
    ax.set_title("PathMNIST Test Confusion Matrix")
...
def train_cnn(
    ...
    if checkpoint_path.exists() and not force_retrain and not resume_from_checkpoint:
        payload = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
        payload["skipped_existing_checkpoint"] = True
        return payload

    if force_retrain and resume_from_checkpoint:
        raise ValueError("force_retrain and resume_from_checkpoint are mutually exclusive")
    ...
    if resume_from_checkpoint:
        ...
        if history_path.exists():
            ...
            if rows:
                start_epoch = max(int(row["epoch"]) for row in rows)
        if start_epoch == 0:
    ...
    if existing_payload is not None:
        ...
        best_state = {k: v.detach().cpu() for k, v in existing_payload["state_dict"].items()}
    ...
    append_log(train_log_path, "PathMNIST CNN training resumed" if resume_from_checkpoint else "PathMNIST CNN training started")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the shared PathMNIST-28 CNN model.")
    ...
    parser.add_argument("--resume-from-checkpoint", action="store_true", help="Continue from the current best validation checkpoint; --epochs is interpreted as additional epochs.")
    parser.add_argument("--scheduler", choices=["step", "plateau", "none"], default="step", help="Learning-rate scheduler to use.")
    parser.add_argument("--no-scheduler", action="store_true", help="Legacy alias for --scheduler none.")
    parser.add_argument("--early-stop-patience", type=int, default=0, help="Stop after this many epochs without validation accuracy improvement; 0 disables early stopping.")
...
def main() -> None:
    ...
    payload = train_cnn(
        ...
        scheduler_type="none" if args.no_scheduler else args.scheduler,
