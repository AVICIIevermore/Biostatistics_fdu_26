...
> **关于测量噪声**：单次 $n_{\text{rep}} = 1$ 运行在 $n \in \{600, 700\}$ 上对 NEW-13 / NEW-25 出现了违反单调性的异常值（n=700 NEW-25 比 n=600 还快 15%），事后用 5 次重复 + 随机打乱顺序 + 显式 `gc()` 的验证脚本（`verify_anomaly.R`）确认是系统噪声。下文所有 $n \in \{600, 700\}$ 上 NEW-13 / NEW-25 的计时数字均替换为 5 次重复的均值（GEXP 那两支以及小 $n$ 的数据未受影响）。
...
| NEW-13        |  $1.42 \pm 0.45$ | $\mathbf{0.055\times}$ |
| NEW-25        |  $2.76 \pm 0.19$ | $\mathbf{0.107\times}$ |

NEW-13 在**同样 $R=13$ 下比 naive `est.cov` 快约 18 倍**，直接验证 Section 3 的乘法闭包确实消去了主导的 $O(R^2 m^3)$ 矩阵乘法。

NEW-13 vs NEW-25 的对比恰好印证了 6.6 节里的复杂度公式：候选核数 $13 \to 25$（$1.92\times$），$t_{\text{cov}}$ 从 1.42 s 增加到 2.76 s（$1.94\times$）——**与 $R$ 近似线性**，而不是 naive 法的 $R^2$ 关系。这意味着把候选核数翻倍只让构造协方差线性变贵，仍然远比 naive 法在同 $R$ 下便宜（NEW-25 在 $R = 25$ 上的 $t_{\text{cov}}$ 还不到 naive 法在 $R = 13$ 上的 $11\%$）。NEW-25 最终保留的子集 $q = 4$ strict（NEW-13 平均 $q \approx 4.65$），因此后续 bootstrap / 检验统计量阶段对 NEW-25 反而稍微便宜。
...
| NEW-13        | $8.20 \pm 2.70$  | $\mathbf{0.18\times}$ |
| NEW-25        | $9.67 \pm 0.71$  | $\mathbf{0.21\times}$ |

NEW-13 比 GEXP-rich-13 快 $5.6\times$、比 GEXP-5 快 $1.9\times$；NEW-25 比 GEXP-rich-13 快 $4.8\times$、比 GEXP-5 快 $1.6\times$。即使把候选核数从论文的 $R = 5$ 拉到 $R = 25$（多 5 倍），端到端仍然比论文方法快，且 power 在 $n \ge 300$ 上和 GEXP 没有显著差别（见 6.4）。

值得注意 NEW-13 的 SD 偏高（$\pm 2.70$ s），原因是它的 $q$ 在不同 batch 上能在 $\{4, 5\}$ 之间小幅波动，这一点点波动叠加系统噪声会被放大；NEW-25 因为算术 grid 更宽，pivoted Cholesky 一致剪到 $q = 4$，反而方差更小（$\pm 0.71$ s）。这也是把 $R$ 调大的一个隐性好处。
...
实测 $t_{\text{cov}}$ 在 $n \ge 400$ 的尾段上，GEXP-rich-13 增长接近 $m^3$（$n: 400\to 700$ 增加 $5.3\times$，理论 $5.4\times$），NEW-13 增长接近 $m^2$（$n: 400\to 700$ 增加 $2.7\times$，理论 $3.1\times$）——略低于理论但仍在 $m^2$ 量级。NEW-13 vs NEW-25 的 $t_{\text{cov}}$ 随 $R$ 线性增长（见 6.2），也吻合公式中 $R\cdot m^2$ 主导项的预测。

### 6.7 运行时间

23 核 Windows 工作站，主体实验（8 个样本量 × 4 个方法 × $n_{\text{iter}} = 200$）端到端壁钟时间 **30.9 分钟**，其中 GEXP-rich-13 一支占了约 60%（预期的瓶颈）。事后做的 5 次重复验证（4 个 $(n, \text{method})$ 对）额外耗时约 15 分钟。
