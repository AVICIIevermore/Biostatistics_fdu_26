...
def extract_embeddings(
    ...
    requested = tuple(layers or ["layer1", "layer2", "final"])
    chunks = {name: [] for name in requested}
    ...
    with torch.no_grad():
        for (xb,) in loader:
            ...
            for name in requested:
                chunks[name].append(out[name].detach().cpu().numpy())
