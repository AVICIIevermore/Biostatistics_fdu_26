...
method_labels <- c(
  ...
  layer1_pool2x2_gaussian5 = "Layer1 2x2 Pooled Gaussian-5",
...
plot_power <- function(power_summary, output_path) {
  method_order <- unique(as.character(power_summary$method))
  ...
  graphics::plot(first$noise_sigma, first$reject_rate, type = "b", pch = method_pchs[method_order[1]], col = method_cols[method_order[1]], lwd = 2, ylim = c(0, 1), xlab = "Noise Sigma", ylab = "Estimated Power", main = "MNIST Power: {1,2,3} vs {1,2,8}")
