experiment_config <- list(
  experiment_name = "faster_mnist_cnn_embedding_power",
  noise_levels = seq(0, 1, length.out = 6),
  n.rep = 10,
  resamp = 100,
  n.iter = 500,
  alpha = 0.05,
  seed = 20260525L,
  set_x = c(1, 2, 3),
  set_y = c(1, 2, 8),
  results_dir = file.path(normalizePath(".", winslash = "/", mustWork = FALSE), "Results"),
  checkpoint_path = file.path("models", "mnist_cnn_checkpoint.pt"),
  train_batch_size = 128L,
  train_epochs = 20L,
  train_lr = 1e-3,
  val_fraction = 5000 / 60000,
  eval_batch_size = 1024L,
  target_val_accuracy = 0.98,
  python_executable = "/home/dechao/.conda/envs/cv-hw2/bin/python",
  python_module = file.path("python", "cnn_pipeline.py"),
  method_order = c(
    "raw_pixel_gaussian5",
    "final_embedding_gaussian5",
    "multilayer_single_gaussian",
    "multilayer_gaussian15"
  )
)
