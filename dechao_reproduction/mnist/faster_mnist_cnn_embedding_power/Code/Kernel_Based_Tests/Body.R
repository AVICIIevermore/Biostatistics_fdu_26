...
checkpoint_path <- file.path(experiment_root, config$checkpoint_path)
...
append_log(log_file, "cnn embedding power run invoked")
...
py_module <- import_from_path("cnn_pipeline", path = file.path(experiment_root, "python"))
append_log(log_file, sprintf("reticulate python=%s", py_config()$python))

train_bundle <- load_mnist_train_flat(file.path(experiment_root, "data"))
test_bundle <- load_mnist_test_flat(file.path(experiment_root, "data"))
...
train_info <- py_module$train_cnn_model(
  data_dir = file.path(experiment_root, "data"),
  ...
  history_path = file.path(experiment_root, "models", "train_history.csv"),
  train_log_path = file.path(experiment_root, "models", "train.log"),
  train_plot_path = file.path(experiment_root, "models", "training_curves.png"),
  config_dump_path = file.path(experiment_root, "models", "train_config.json"),
  force_retrain = TRUE
)
append_log(log_file, sprintf(
...
if (start_iter <= config$n.rep) {
  for (iter in start_iter:config$n.rep) {
    ...
    for (k in seq_along(config$noise_levels)) {
      ...
      embed.X <- extract_cnn_embeddings(py_module, checkpoint_path, noisy.X, config$eval_batch_size)
      embed.Y <- extract_cnn_embeddings(py_module, checkpoint_path, noisy.Y, config$eval_batch_size)
...
utils::write.csv(power_df, file.path(config$results_dir, "mnist_cnn_power_results.csv"), row.names = FALSE)
utils::write.csv(sigma_df, file.path(config$results_dir, "mnist_cnn_sigma_diagnostics.csv"), row.names = FALSE)
append_log(log_file, "cnn power CSV outputs written")
