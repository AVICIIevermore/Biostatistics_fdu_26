...
old_root <- normalizePath(file.path(output_dir, "..", "..", "mnist_additive_noise_type1_same_distribution"), mustWork = TRUE)
...
append_line(comparison_path, sprintf("Type-I equivalence check: sigma=%.3f, n_rep=%d, B_boot=%d", sigma_value, n_rep_check, B_boot_check))
...
for (scenario_idx in seq_along(config$scenarios)) {
  ...
  for (kernel_name in kernel_choice[3:5]) {
    bootstrap_inputs$multi[[kernel_name]] <- lapply(seq_len(n_rep_check), function(i) {
      set.seed(43000L + scenario_idx * 1000L + match(kernel_name, kernel_choice) * 100L + i)
      MASS::mvrnorm(B_boot_check, mu = rep(0, resamp), Sigma = diag(2, nrow = resamp, ncol = resamp))
    })
  }

  scenario_inputs[[scenario$name]] <- list(
    sigma_value = sigma_value,
    n_rep_check = n_rep_check,
    B_boot_check = B_boot_check,
    set_x = scenario$set_x,
    set_y = scenario$set_y,
    resamp.x = resamp.x,
    resamp.y = resamp.y,
    bootstrap_inputs = bootstrap_inputs,
    noise_seeds = c(
      data_x = 41000L + scenario_idx * 10L + 1L,
      data_y = 41000L + scenario_idx * 10L + 2L
    )
  )

  append_line(comparison_path, sprintf("scenario %s", scenario$name))
  results[[scenario$name]] <- list(single = list(), multi = list())

  for (kernel_name in kernel_choice[1:2]) {
    results[[scenario$name]]$single[[kernel_name]] <- vector("list", n_rep_check)
    for (i in seq_len(n_rep_check)) {
      ...
      results[[scenario$name]]$single[[kernel_name]][[i]] <- list(old = old_obj, fast = fast_obj, compare = cmp)
  ...
  for (kernel_name in kernel_choice[3:5]) {
    results[[scenario$name]]$multi[[kernel_name]] <- vector("list", n_rep_check)
    for (i in seq_len(n_rep_check)) {
      X <- as.matrix(data.X[resamp.x[i, ],, drop = FALSE])
      Y <- as.matrix(data.Y[resamp.y[i, ],, drop = FALSE])
      ...
      results[[scenario$name]]$multi[[kernel_name]][[i]] <- list(old = old_obj, fast = fast_obj, compare = cmp)
      overall_pass <- overall_pass && cmp$pass
...
saveRDS(scenario_inputs, file.path(output_dir, "inputs.rds"))
