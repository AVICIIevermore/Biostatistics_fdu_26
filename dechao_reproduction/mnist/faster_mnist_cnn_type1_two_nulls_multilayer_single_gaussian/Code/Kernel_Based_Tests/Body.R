...
required_layers_for_methods <- function(method_order){
  unique(unlist(lapply(method_order, function(method) {
    switch(
      method,
      layer1_gaussian5 = "layer1",
      layer1_pool2x2_gaussian5 = "layer1_pool2x2",
...
append_log(log_file, "cnn embedding type-I two-null run invoked")
append_log(log_file, sprintf(
  "target config: scenarios=%s; noise_levels=%s; n.rep=%d; resamp=%d; n.iter=%d; alpha=%.3f; methods=%s",
  paste(vapply(config$scenarios, `[[`, character(1), "name"), collapse = ","),
  paste(config$noise_levels, collapse = ","), config$n.rep, config$resamp, config$n.iter, config$alpha,
  paste(config$method_order, collapse = ",")
))
...
required_layers <- required_layers_for_methods(config$method_order)
if (length(required_layers) == 0) required_layers <- character(0)
...
if (file.exists(checkpoint_file)) {
  ...
  completed_keys <- ckpt$completed_keys
  append_log(log_file, sprintf("resuming from checkpoint: completed scenario-iter keys=%d", length(completed_keys)))
}

for (scenario_idx in seq_along(config$scenarios)) {
  scenario <- config$scenarios[[scenario_idx]]
  scenario_name <- scenario$name
  selected <- select_digit_pools(test_bundle$test.x, test_bundle$test.label, scenario$set_x, scenario$set_y)
  pool.X <- selected$data.X
  pool.Y <- selected$data.Y
  append_log(log_file, sprintf("scenario %s selected test pools: |X|=%d, |Y|=%d", scenario_name, nrow(pool.X), nrow(pool.Y)))

  for (iter in seq_len(config$n.rep)) {
    key <- paste(scenario_name, iter, sep = "::")
    if (key %in% completed_keys) {
      append_log(log_file, sprintf("scenario %s iter %d/%d skipped from checkpoint", scenario_name, iter, config$n.rep))
      next
    }

    iter_start <- Sys.time()
    append_log(log_file, sprintf("scenario %s iter %d/%d started", scenario_name, iter, config$n.rep))
    set.seed(config$seed + 100000L * scenario_idx + 1000L * iter)
    ...
    for (k in seq_along(config$noise_levels)) {
      ...
      append_log(log_file, sprintf("scenario %s iter %d/%d sigma %d/%d = %.3f started", scenario_name, iter, config$n.rep, k, length(config$noise_levels), sigma))
      ...
      out$power_rows$scenario <- scenario_name
      out$power_rows <- out$power_rows[, c("scenario", setdiff(names(out$power_rows), "scenario"))]
      out$sigma_rows$scenario <- scenario_name
      out$sigma_rows <- out$sigma_rows[, c("scenario", setdiff(names(out$sigma_rows), "scenario"))]
      iter_power_rows[[length(iter_power_rows) + 1L]] <- out$power_rows
      iter_sigma_rows[[length(iter_sigma_rows) + 1L]] <- out$sigma_rows
      append_log(log_file, sprintf("scenario %s iter %d/%d sigma %.3f finished", scenario_name, iter, config$n.rep, sigma))
    }
    ...
    append_log(log_file, sprintf("scenario %s iter %d/%d finished in %s; checkpoint updated", scenario_name, iter, config$n.rep, format(Sys.time() - iter_start)))
  }
}
...
append_log(log_file, sprintf("all requested scenario iterations available in %s", format(Sys.time() - start)))
